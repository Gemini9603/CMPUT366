...
class MRV(VarSelector):
    ...
    """
    def select_variable(self, grid):
        min_domain = "0123456789"    #set arbitrary domain values that is more than the total number of possible domain values
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                domain = grid.get_cells()[i][j]
                if len(domain)>1 and len(domain)<len(min_domain):
                    min_domain = domain
                    mrv = (i,j)
        return mrv
...
class AC3:
    ...
    def pre_process_consistency(self, grid):
        ...
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                domain = grid.get_cells()[i][j]
                if len(domain)==1:
                    variables_assigned.add((i,j))
        success = self.consistency(grid, variables_assigned)
        return success
    ...
    def consistency(self, grid, Q):
        ...
        while len(Q)!=0:
            var = Q.pop()
            var_row, failure_row = self.remove_domain_row(grid, var[0], var[1])
            var_column, failure_column = self.remove_domain_column(grid, var[0], var[1])
            var_unit, failure_unit = self.remove_domain_unit(grid, var[0], var[1])
            
            if failure_row==True or failure_column==True or failure_unit==True:
                return False
    ...
    def search(self, grid, var_selector):
        ...
        for d in grid.get_cells()[i][j]:
            if grid.is_value_consistent(d, i, j):
                copy_grid = grid.copy()
                copy_grid.get_cells()[i][j] = str(d)
                Q = {(i,j)}
                success = ac3.consistency(copy_grid, Q)
                if success is True:
                    rb = self.search(copy_grid, var_selector)
                    if rb is True:
                       return rb
                grid.get_cells()[i][j] = grid.get_cells()[i][j].replace(d,'')   
        return False
...
for p in problems:
    ...
    if success is False:
        print("There is no solution for this grid")        
    print("Solving grid " + str(total) + " using MRV heuristic")
    solution = bt.search(g, mrv)
    end_time_mrv = time.time()
    total_time_mrv = end_time_mrv - start_time_mrv
    rt_mrv.append(total_time_mrv)
    if solution:
        print("Solved")
        print()
    else:
    ...
    if success is False:
        print("There is no solution for this grid")       
    print("Solving grid " + str(total) + " using First Available heuristic") 
    solution = bt.search(g, fa)
    end_time_fa = time.time()
    total_time_fa = end_time_fa - start_time_fa
    rt_fa.append(total_time_fa)
    if solution:
        print("Solved")
        print()
    else:
...
plotter = PlotResults()
plotter.plot_results(rt_mrv, rt_fa, "Running Time Backtracking (MRV)", "Running Time Backtracking (FA)", "running_time")