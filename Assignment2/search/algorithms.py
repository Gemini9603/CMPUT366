...
class CBSState:
    ...
    def successors(self):
        ...
        """
        Boolean, state = self.is_solution()
        children = []
        count = 0
        for agent, path in self._paths.items():
            for point in path:                
                time = point.get_g()
                check = (point,time)
                if point==state[0] and time==state[1]:
                    child = CBSState(self._map, self._starts, self._goals)
                    child._constraints = copy.deepcopy(self._constraints)
                    child.set_constraint(point, time, agent)
                    children.append(child)                    
                    count+=1
                    break #since an agent cannot have a conflict with itself there
                          #is no point in iterating through the rest of the path 
            if count>=2:
                break
        return children
...
class CBS():
    def search(self, start):
        ...
        while len(open)>0:
            ...
            Boolean, state = node.is_solution()
            if Boolean==True and state==None:
                return node._paths, node._cost
